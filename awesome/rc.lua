...
globalkeys = gears.table.join(
	...
	end, { description = "Hide topbar", group = "panel" }),
	...
	awful.key({ modkey, "Shift" }, "Tab", function()
		local current_layout = io.popen("setxkbmap -query | grep layout | awk '{print $2}'"):read("*all")

		if current_layout:match("us") then
	...
	end, { description = "Lower screen brightness", group = "launcher" }),

	awful.key({}, "XF86MonBrightnessUp", function()
		awful.spawn("brightnessctl set 10%+")
	end, { description = "Raise screen brightness", group = "launcher" }),
	...
	awful.key({ modkey }, "Escape", function()
		awful.spawn("warpd --normal")
	end, { description = "open warpd", group = "launcher" }),
	awful.key({ modkey }, ";", function()
		awful.spawn("warpd --history")
	end, { description = "open warpd history", group = "launcher" }),
	awful.key({ modkey }, "f", function()
		awful.spawn("warpd --hint")
	end, { description = "warpd hints", group = "launcher" }),
	awful.key({ modkey, "Shift" }, "f", function()
		awful.spawn("warpd --hint2")
	end, { description = "warpd hints2", group = "launcher" }),
	...
	awful.key({ modkey, "Shift" }, "space", function()
		awful.layout.inc(-1)
	end, { desciption = "select previous", group = "layout" }),
...
awful.rules.rules = {
	...
	{ rule = { class = "sysmon" }, properties = { screen = 1, tag = "9", urgent = false } },
}
-- }}}
...
client.connect_signal("property::urgent", function(c)
	if c.class == "sysmon" then
		c.urgent = false
	end
end)

client.connect_signal("sleep", function(c)
	awful.spawn("slock")
end)
